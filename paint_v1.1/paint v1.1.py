...
def Shapes(event,x,y,flags,params):
    ...
    if (flags & cv2.EVENT_FLAG_CTRLKEY) and cv2.getTrackbarPos("Brush",shapes) == 1: #first change trackbar pos and then press CTRL
    ...
    if event==cv2.EVENT_LBUTTONDOWN and  cv2.getTrackbarPos("Brush",shapes)==1 : 
            drawing=True
...
def main():
    ...
    while True:
        ...
        if cv2.getTrackbarPos("Brush","Shapes")==1:
            
            cv2.namedWindow("Brush")

            brush_thickness=cv2.getTrackbarPos("Thickness:","Brush")
            brush_thickness=1 if brush_thickness==-1 else brush_thickness

            brush_red,brush_green,brush_blue = returnRGB("Brush")

            if drawing:
                cv2.circle(blank_img,(x1,y1),brush_thickness,(brush_blue,brush_green,brush_red),-1)
            else:
                x1=-100
                y1=-100
